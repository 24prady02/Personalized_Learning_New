...
def build_state_vector(lp_state: Optional[Dict],
                       ...
                       mastery: float = 0.30,
                       hvsae_latent=None) -> List[float]:
    """Build a state vector from LP + channel state.

    Default (hvsae_latent=None): 12-d hand-coded features. Preserves
    original contract so existing callers continue to work unchanged.

    With hvsae_latent provided (256-d tensor, numpy array, or list):
    returns [hvsae_latent_256 || existing_12d] = 268-d. Lets the
    LPProgressionRNN condition on HVSAE's semantic state encoding in
    addition to the categorical summary — the RNN's input_dim just has
    to be bumped to 268 when re-training. Existing RNN weights keep
    working in the 12-d mode.
    """
    ...
    # Normalise hvsae_latent to a flat list of 256 floats.
    try:
        import torch as _t
        if isinstance(hvsae_latent, _t.Tensor):
            vec = hvsae_latent.detach().view(-1).tolist()
        else:
            vec = list(hvsae_latent)
            if vec and isinstance(vec[0], (list, tuple)):
                vec = [float(x) for row in vec for x in row]
            else:
                vec = [float(x) for x in vec]
    except Exception:
        return base  # fail-safe: fall back to 12-d

    if len(vec) > 256:
        vec = vec[:256]
    elif len(vec) < 256:
        vec = vec + [0.0] * (256 - len(vec))
    return vec + base  # 268-d
