    ...
    def __init__(self, config: Dict):
        ...
        if LOCAL_CLIENT_AVAILABLE:
            try:
                ...
                # Always prefer the local client. If the graph is empty we'd rather
                # get an empty result (returned instantly) than block for seconds on
                # every query waiting for a SPARQL endpoint that isn't running.
                self.use_local_only = True
                has_data = bool(self.local_client.graph) and len(self.local_client.graph.nodes()) > 0
                print(f"[CSE-KG] Using LOCAL graph "
                      f"({len(self.local_client.graph.nodes()) if has_data else 0} nodes, "
                      f"SPARQL disabled)")
