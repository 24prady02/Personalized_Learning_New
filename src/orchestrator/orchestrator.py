...
class InterventionOrchestrator:
    ...
    def _select_intervention(self, student_profile: Dict, context: Dict) -> Dict:
        ...
        Fix 5b: when RL recommender is unavailable (no trained policy), use a
        principled rule-based scorer grounded in mastery, emotion, and learning
        style — not random Q-values. Every rule has a cited rationale."""
        recommender = self.models.get('intervention_recommender')
        if recommender:
            recommendations = recommender.recommend(student_profile, context, top_k=3)
            if recommendations:
    ...
    def _generate_content(self, latent: torch.Tensor,
                         ...
                         three_channel_result: Dict = None) -> Dict:
