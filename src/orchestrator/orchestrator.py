...
class InterventionOrchestrator:
    ...
    def __init__(self, config: Dict, models: Dict, use_rl: bool = True, use_hierarchical_rl: bool = False):
        ...
        # ⭐ Enhanced Personalized Generator with Groq API
        try:
            import os
            ...
            if groq_api_key:
                ...
                self.models['groq_client'] = groq_client
                self.models['enhanced_generator'] = EnhancedPersonalizedGenerator(groq_client)
                print("[OK] Enhanced Personalized Generator with Groq initialized")
            else:
                print("[WARN] Groq API key not found - Enhanced generator not available")
        except Exception as e:
            print(f"[WARN] Enhanced generator initialization failed: {e}")
    ...
    def _encode_session(self, session_data: Dict) -> Dict:
        """Encode session using HVSAE"""
        hvsae = self.models.get('hvsae')
        
        if hvsae is None:
            # Return dummy encoding if HVSAE not available
            return {
                'latent': torch.zeros(1, 256),  # Dummy latent
                'mu': torch.zeros(1, 256),
                'kappa': torch.ones(1, 1),
                'attention_weights': None,
                'misconception_probs': torch.zeros(1, 5)
            }
    ...
    def _diagnose_cognition(self, student_id: str, session_data: Dict) -> Dict:
        """Cognitive diagnosis using Student State Tracker"""
        if self.state_tracker is None:
            return {'mastery_profile': {}, 'knowledge_gaps': []}
        
        # Get student's knowledge state from tracker
        knowledge_state = self.state_tracker.get_knowledge_state(student_id)
        mastery_profile = knowledge_state.get('concept_mastery', {})
        
        # Identify knowledge gaps (mastery < 0.5)
        knowledge_gaps = []
        for concept, mastery in mastery_profile.items():
            # Handle dict, list, float, and None mastery values
            if isinstance(mastery, dict):
                mastery_value = mastery.get('mastery', mastery.get('value', 0.5))
            elif isinstance(mastery, list):
                # If it's a list, take the first element or average
                if len(mastery) > 0:
                    mastery_value = float(mastery[0]) if isinstance(mastery[0], (int, float)) else 0.5
                else:
                    mastery_value = 0.5
    ...
    def _analyze_behavior(self, session_data: Dict) -> Dict:
        """Behavioral analysis using RNN/HMM"""
        ...
        if rnn is not None:
            ...
            time_deltas = torch.tensor([session_data.get('time_deltas', [0]*len(action_sequence))]).unsqueeze(-1)
            outcomes = torch.zeros_like(action_ids)  # Would need actual outcomes
            lengths = torch.tensor([len(action_sequence)])
    ...
    def _identify_gaps(self, student_id: str, 
        ...
        """Identify knowledge gaps using CSE-KG and student graph"""
        cse_kg = self.models.get('cse_kg_client')
        
        if cse_kg is None:
            return []
        
        # Extract concepts from code/error
        concept_retriever = self.models.get('concept_retriever')
        if concept_retriever:
            concepts = concept_retriever.retrieve_from_code(
        ...
        # Identify gaps
        gaps = []
        mastery = cognitive_assessment.get('mastery_profile', {})
        
        for concept in concepts:
            # Check mastery
            if concept in mastery and mastery[concept] < self.intervention_threshold:
                # Get prerequisites
                prereqs = cse_kg.get_prerequisites(concept)
    ...
    def _select_intervention(self, student_profile: Dict, context: Dict) -> Dict:
        ...
        if recommender is None:
            return {'type': 'visual_explanation', 'priority': 0.5}
        
        # Get recommendations
        recommendations = recommender.recommend(
            student_profile, context, top_k=3
        )
        ...
        top_intervention, score, rationale = recommendations[0]
        
        # Calculate priority based on multiple factors
        priority = self._calculate_priority(context, score)
        
        return {
            'type': top_intervention,
            'priority': priority,
            'confidence': score,
            'rationale': rationale,
    ...
    def _generate_content(self, latent: torch.Tensor,
                         ...
                         behavioral_analysis: Dict = None) -> Dict:
        """
        Generate personalized content using Adaptive Explanation Generator
        Integrated with HVSAE, Groq, and RL
        """
            ...
            try:
                ...
                if 'enhanced_generator' not in self.models:
                    try:
                        ...
                        if groq_api_key:
                            ...
                            self.models['groq_client'] = groq_client
                            self.models['enhanced_generator'] = EnhancedPersonalizedGenerator(groq_client)
                            print("[OK] Enhanced Personalized Generator initialized with Groq")
                    except Exception as e:
                ...
                if self.models.get('enhanced_generator'):
                    try:
                        ...
                        print("[OK] Response generated with Enhanced Personalized Generator (Groq API)")
                    except Exception as e:
                        ...
                        try:
                            enhanced_response = self._enhance_with_groq(
                            ...
                            print(f"[WARN] Groq enhancement also failed: {e2}")
    ...
    def _enhance_with_groq(self, base_explanation: str, student_data: Dict,
                           adaptive_result: Dict, student_profile: Dict) -> str:
        """Enhance explanation using Groq API"""
        try:
            from groq import Groq
            import os
            ...
            prompt = f"""You are an expert programming tutor. Enhance this explanation to be more personalized and clear.

STUDENT CONTEXT:
- Question: {student_data.get('question', 'Help with code')}
- Code: {student_data.get('code', 'N/A')[:200]}
- Error: {student_data.get('error_message', 'None')}

STUDENT PROFILE:
- Learning Style: {student_profile.get('learning_style', {}).get('visual_verbal', 'visual')}
- Cognitive State: {adaptive_result.get('learning_style_adaptation', {}).get('visual_verbal', 'engaged')}
- Mastery: {adaptive_result.get('prior_knowledge', {}).get('average_mastery', 0.5):.0%}

BASE EXPLANATION:
{base_explanation}

Enhance this explanation to be:
...
Generate the enhanced explanation:"""
            ...
            completion = groq.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert, encouraging programming tutor."},
            ...
            print(f"[WARN] Groq enhancement error: {e}")
            return base_explanation
    ...
    def _calculate_complete_metrics(self, session_data: Dict, adaptive_result: Dict,
        ...
        if self.metrics_calculator:
            ...
            print(f"[Metrics] ✅ BERT quality: score={bert_quality.get('quality_score', 0):.2f}")
        else:
            # Fallback if calculator not available
    ...
    def _get_personality_profile(self, student_id: str, session_data: Dict) -> Dict:
        ...
        if profiler:
            behavioral_data = {
                'exploration_rate': 0.5,  # Would compute from history
                'persistence': 0.6,
                'organization': 0.7
            }
    ...
    def _infer_learning_style_from_behavior(self, session_data: Dict) -> Dict:
        ...
        Uses LearningStyleAssessor if available
        """
        try:
            ...
            inferred = assessor.infer_from_behavior(behavioral_patterns)
    ...
    def _action_to_id(self, action: str) -> int:
        ...
        action_types = self.config['behavioral']['action_types']
        try:
