...
class EnhancedPersonalizedGenerator:
    ...
    def _build_enhanced_prompt(
        ...
        # Fallback if no structured KG info
        ...
        if kg and not (cse_kg_info or pedagogical_kg_info or coke_info):
            ...
            prompt_parts.append(f"Common misconceptions: {kg.get('common_misconceptions', 'N/A')}")
            prompt_parts.append(f"Better mental model: {kg.get('better_mental_model', 'N/A')}")
        ...
        prompt_parts.append("2. Use the appropriate tone and style based on all adaptations above")
        prompt_parts.append("3. Include relevant examples from their interests")
        prompt_parts.append("4. Acknowledge progress if significant improvement")
        prompt_parts.append("5. Provide learning strategy tips if applicable")
        prompt_parts.append("6. Match the difficulty and pacing to their level")
        prompt_parts.append("7. Be encouraging and personalized")
        prompt_parts.append("8. **CRITICAL: Ground your explanation in the knowledge graphs provided above**")
        prompt_parts.append("   - Reference CSE-KG concepts and prerequisites explicitly")
        prompt_parts.append("   - Address Pedagogical KG misconceptions directly")
        prompt_parts.append("   - Adapt to COKE cognitive state (e.g., if confused, use simpler language)")
