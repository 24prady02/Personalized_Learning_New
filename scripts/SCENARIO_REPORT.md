...
- [OK] **0.1** _completely empty_  (2.66s)
    - output: `{"resolved": [["unknown", 0.0]], "stage": 1}`
- [OK] **0.2** _whitespace-only_  (0.0s)
    - output: `{"resolved": [["unknown", 0.0]]}`
- [OK] **0.3a** _single token '?'_  (0.12s)
    - output: `{"top": "unknown", "encoding_strength": "partial"}`
- [OK] **0.3b** _single token '😭'_  (0.08s)
    - output: `{"top": "unknown", "encoding_strength": "partial"}`
- [OK] **0.3c** _single token '.'_  (0.09s)
    - output: `{"top": "unknown", "encoding_strength": "partial"}`
- [OK] **0.3d** _single token 'k'_  (0.08s)
    - output: `{"top": "unknown", "encoding_strength": "partial"}`
- [OK] **0.5a** _idk variant 'idk'_  (0.11s)
    - output: `{"confidence": 0.3, "lp": "L1"}`
- [OK] **0.5b** _idk variant "i don't know"_  (0.07s)
    - output: `{"confidence": 0.3, "lp": "L1"}`
- [OK] **0.5c** _idk variant 'no clue'_  (0.06s)
    - output: `{"confidence": 0.3, "lp": "L1"}`
- [OK] **0.5d** _idk variant 'no idea'_  (0.05s)
    - output: `{"confidence": 0.3, "lp": "L1"}`
...
- [OK] **1.6** _detect assignment_vs_compare_  (0.02s)
...
- [OK] **1.7** _detect integer_division_  (0.02s)
...
- [OK] **1.9** _detect array_index_  (0.02s)
...
- [OK] **1.11** _detect array_not_allocated_  (0.02s)
...
- [OK] **1.19** _detect overloading_  (0.02s)
...
- [OK] **1.22** _three concepts in one msg_  (0.02s)
...
- [ok] **6.1** _label=frustrated_  (0.05s)
...
- [ok] **6.8** _label=engaged_  (0.04s)
    - output: `{"expected_label": "engaged", "attribution": "neutral", "self_efficacy": "growth", "high_anxiety": false, "imposter_flag": true, "srl_phase": "unknown", "lang_imposter": false}`

## Layer 7 — Behavioral actions (resolver smoke)

- [OK] **7.1** _trial-and-error_  (0.07s)
    - output: `{"top": "unknown", "stage": 1}`
- [OK] **7.2** _systematic debug_  (0.07s)
    - output: `{"top": "unknown", "stage": 1}`
- [OK] **7.3** _long pause stuck_  (0.08s)
    - output: `{"top": "unknown", "stage": 1}`
- [OK] **7.5** _manic burst_  (0.08s)
    - output: `{"top": "unknown", "stage": 1}`
- [OK] **7.6** _read-only_  (0.06s)
    - output: `{"top": "unknown", "stage": 1}`
- [OK] **7.7** _help-avoidant_  (0.1s)
    - output: `{"top": "unknown", "stage": 1}`
...
- [OK] **9.8** _pseudocode_  (0.02s)
    - output: `{"top": "unknown", "conf": 0.0}`
- [OK] **9.9** _sql/script text_  (0.02s)
    - output: `{"top": "unknown", "conf": 0.0}`
...
- [ok] **11.7** _new concept first encounter_  (0.07s)
    - output: `{"intervention": "worked_example", "imposter_flag": true, "self_efficacy": "neutral", "attribution": "neutral"}`
...
- [ok] **15.3** _internal healthy_  (0.04s)
...
- [ok] **15.4** _internal unhealthy_  (0.04s)
...
- [ok] **15.8** _factual question_  (0.04s)
...
- [OK] **18.2** _imposter + external combo_  (0.04s)
...
- [OK] **18.6** _WM threshold boundary_  (0.05s)
...
- [OK] **19.1** _DBStore upsert+read roundtrip_  (0.02s)
    - output: `{"stored": [0.42, "2026-05-22T03:51:07+00:00"]}`
- [OK] **19.2** _DB handles 10-thread concurrent writes_  (0.08s)
...
- [OK] **19.4** _A/B assignment is sticky per student_  (0.02s)
    - output: `{"calls": ["verbose", "verbose", "verbose"]}`
- [OK] **19.5** _A/B balanced across 200 students_  (0.11s)
...
- [OK] **19.7** _GDPR export contains all fields_  (0.04s)
    - output: `{"export_keys": ["student_id", "mastery", "state", "consent", "variants", "audit", "exported_at"]}`
- [OK] **19.8** _auth token roundtrip_  (0.02s)
...
- [OK] **19.15** _decay after 100 days approaches prior_  (0.04s)
    - output: `{"fresh": 0.99, "after_100d": 0.156, "prior": 0.15}`

## Layer 20 — Progression reporting

- [OK] **20.1** _progression_for filters by skill + preserves order_  (0.02s)
...
- [OK] **20.8** _trajectory empty for unknown student_  (0.02s)
    - output: `{"trajectory": []}`
- [OK] **20.9** _full simulated 6-turn session flow_  (0.02s)
...
- [OK] **20.10** _intervention captured per-turn AND aggregated_  (0.02s)
...
- [OK] **20.11** _session_id preserved in turn audit_  (0.02s)
    - output: `{"sessions": ["s1", "s1", "s1", "s2", "s2"]}`
- [OK] **20.12** _dwell_s preserved per turn_  (0.02s)
    - output: `{"dwells": [null, 8.4, 32.1, 60.0, 5.2]}`

## Layer 21 — Behavioral + KG signals per turn

- [OK] **21.1** _reasoning length+complexity per turn_  (0.02s)
...
- [OK] **21.3** _KG concept + counts per turn_  (0.02s)
...
- [OK] **21.4** _COKE cognitive_state per turn_  (0.02s)
    - output: `{"states": ["confused", "engaged", "frustrated"]}`
- [OK] **21.5** _KG block sizes per turn_  (0.02s)
    - output: `{"sizes": [1200, 40]}`
- [OK] **21.6** _old + new fields coexist in one turn_  (0.02s)
