...
class Trainer:
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch in pbar:
            ...
            # === HVSAE Forward ===
            if 'hvsae' in self.models:
                hvsae_outputs = self.models['hvsae'](batch, compute_graph=False)
                hvsae_losses = self.models['hvsae'].compute_loss(hvsae_outputs, batch)
            ...
            if 'behavioral_rnn' in self.models:
                behavioral_outputs = self.models['behavioral_rnn'](
                    ...
                    batch['timestamps'],
                    torch.zeros_like(batch['action_sequence']),  # Outcomes
                ...
                if behavioral_targets:
                    behavioral_losses = self.models['behavioral_rnn'].compute_loss(
                        behavioral_outputs,
                        behavioral_targets
                    )
                    ...
                    behavioral_losses['total'].backward()
                    self.optimizers['behavioral_rnn'].step()
                    
                    total_losses['behavioral'] += behavioral_losses['total'].item()
...
def main():
    ...
    if config['data']['datasets']['assistments']['enabled']:
        try:
            ...
            models['dina'] = DINAModel(num_concepts=num_concepts, config=config)
